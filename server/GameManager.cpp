...
Client* GameManager::findDisconnected(string name){
...
void GameManager::removeClientByName(string name){
    cout<< "Mažu klienta " << endl;
    Client *c = GameManager::findDisconnected(name);
    Match *m;
    string msg;
    ...
    if(c->getState()==ClientState::LOBBY){
        disconnectedMap.erase(c->getName());
        c->setExisting(false);
        delete(c);
    }
        ...
        msg = "LEFT;";
        if(std::strcmp(m->getClient1()->getName().c_str(), name.c_str()) == 0){
            MessageManager::sendMessage(msg, m->getClient2()->getSocketID());
            m->getClient2()->setState(ClientState::LOBBY);
        }
        else if(strcmp(m->getClient2()->getName().c_str(), name.c_str()) == 0){
        ...
        else if(std::strcmp(m->getClient2()->getName().c_str(), name.c_str()) == 0){
            MessageManager::sendMessage(msg, m->getClient1()->getSocketID());
            m->getClient1()->setState(ClientState::LOBBY);
        }
        delete(m->getGame());
...
void GameManager::removeClientByID(int socketID){
    cout<< "Mažu klienta s ID: " << socketID << endl;
    Client *c = GameManager::findClient(socketID);
    Match *m;
    string msg;
    
    if(c == NULL){
        Server::closeSocket(socketID);
        return;
    }
...
void GameManager::reconnectClient(int socket){
    ...
    cout << "Klient: " << c->getName() << endl;
    cout << "SocketID: "<< c->getSocketID() << endl;
    cout << "matchID: " << c->getMatchID() << endl;
    
    cout << "V mapě jsou: " << endl;
    ...
    if(c->getState()==ClientState::LOBBY){
        ...
        sleep(1);
        GameManager::sendGame(c->getSocketID());
        msg = "BACK;";
        Match *m = GameManager::findMatch(c->getMatchID());
        if(m->getClient1()->getSocketID() == socket){
            MessageManager::sendMessage(msg, m->getClient2()->getSocketID());
        }
        else if(m->getClient2()->getSocketID() ==  socket){
            MessageManager::sendMessage(msg, m->getClient1()->getSocketID());
        }
        ...
        if(c->getColor()==COLOR::WHITE) msg+="W;";
        else msg += "B;";
        MessageManager::sendMessage(msg, c->getSocketID());
    }
}
...
void GameManager::moveToDisconnected(int socketID){
    cout << "Přesouvam klienta s ID: " << socketID << " do disconnected" << endl;
    ...
    if(it != clientMap.end()){
        ...
        if(c->getState()==ClientState::ROOM){
            ...
            if(mit != matchMap.end()){
                ...
                msg += GameManager::findMatches();
                MessageManager::sendToAll(msg);
                c->setState(ClientState::LOBBY);
            }
        }
        else if(c->getState()==ClientState::GAME){
            Match *m = GameManager::findMatch(c->getMatchID());
            string msg = "PAUSE;";
            if(std::strcmp(m->getClient1()->getName().c_str(), c->getName().c_str()) == 0){
                MessageManager::sendMessage(msg, m->getClient2()->getSocketID());
            }
            else if(strcmp(m->getClient2()->getName().c_str(), c->getName().c_str()) == 0){
                MessageManager::sendMessage(msg, m->getClient1()->getSocketID());
            }
        }
    }
