...
void MessageManager::resolveMessage(string msg, int socket){
    ...
    if(i<0 || i>20){
        GameManager::removeClientByID(socket);
        return;
    }
    if (!MessageManager::validateMessage(msg)){
        GameManager::removeClientByID(socket);
        return;
    }
    ...
    if(strcmp(tok.c_str(), "EXIT") == 0){
    ...
    else if(strcmp(tok.c_str(), "PING") == 0){
        ...
        if(result > -1) {
            ...
            if(match->getClient1()->getTurn()){
                msg = "TURN;T;";
                MessageManager::sendMessage(msg, match->getClient1()->getSocketID());
                msg = "TURN;F;";
                MessageManager::sendMessage(msg, match->getClient2()->getSocketID());
            }
            else{
                msg = "TURN;T;";
                MessageManager::sendMessage(msg, match->getClient2()->getSocketID());
...
bool MessageManager::validateMessage(string msg){
    for(char& c : msg) {
