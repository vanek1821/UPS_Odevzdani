			...
			for(int j = 1; j <= Constants.CHESS_SIZE; j++) {
				...
				box.setOnMouseClicked(event -> 
				{
					if(client.isOpponentConnected()) {
						...
						client.getStageController().setStatus("Clicked on row: "+ row + " and col: " + col);
					}
				});
	...
	private void login() throws UnknownHostException, IOException {
		try {
			if(loginField.getText() != null) {
				...
				try {
					InetSocketAddress isa = new InetSocketAddress(client.getAddressString(), client.getPort());
					socket.connect(isa, 1000);
				}catch(Exception e) {
					Platform.runLater(() -> {
						Alert alert = new Alert(AlertType.WARNING);
						alert.setHeaderText("Can`t connect");
						alert.setContentText("Can`t connect to server, please try again later.");
						alert.showAndWait();
						//System.exit(0);
					});
				}
